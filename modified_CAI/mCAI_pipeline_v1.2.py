...
# coding:utf-8
# =========================================================================================================
# Yingying Dong.2019-11-15. mCAI pipeline v1.2. The weight from genes that high expression and high
#   translation level.The gene id from ribo_num.R code.
# =========================================================================================================
import os
import sys
import argparse
...
parser = argparse.ArgumentParser(description='mCAI pipeline v1.2.The weight from hE_hT genes.', prog='mCAI_hE_hT', usage='%(prog)s [options]')
...
id_path = '/home/hp/Desktop/other_riboseq/{}/experiment3/aligned/ribo_num/'.format(args.spe)

id_file = open('{}hE_hT_only_geneID.txt'.format(id_path), 'r')
DNA_path = '/media/hp/disk1/DYY/reference/annotation/{}/ref/'.format(args.spe)
DNA = open('{}CDS_DNA.fa'.format(DNA_path), 'r')
ex_seq = open('{}hE_hT_seq.fa'.format(DNA_path), 'w')
...
in_file = open('{}hE_hT_seq.fa'.format(DNA_path), 'r')
...
r_script = '''
gene_fre = read.table("hE_hT_codon_fre_RSCU.txt",header = T,sep = '\t',quote = "")
df <- gene_fre
...
weight_file = open("hE_hT_RSCU_weight.txt", 'r')
CAI_file = open('{}{}_mCAI_hE_hT.txt'.format(DNA_path, args.spA), 'w')
...
os.chdir(DNA_path)
os.system('sed -i \'s/>//g\' {}_mCAI_hE_hT.txt'.format(args.spA))
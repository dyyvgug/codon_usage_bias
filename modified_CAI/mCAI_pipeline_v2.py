...
parser = argparse.ArgumentParser(description='mCAI pipeline v2.The weight from ribosome genes.', prog='mCAI_ribo', usage='%(prog)s [options]')
...
for i in gtf:
    gtf_table.append(i.strip().split('\t'))
for k in gtf_table:
    # print(k[9])
    name_id[k[9]] = k[8]

for key in name_id.keys():
    if(keyword.match(key)):
        #print(key +'\t'+ name_id[key])
        id_table.append(name_id[key])
        id_file.write(name_id[key] + '\n')

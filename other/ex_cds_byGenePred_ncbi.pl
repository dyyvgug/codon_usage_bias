...
use List::Compare;
use strict;
# This initial script was written by Prof.Yunkun Dang, annatation and modification was written by Yingying Dong.
...
#---------------------------------------------------------------------------------------------------------------------------
mkdir "ref";
my $ge ; my %genome; my $species = "Homo_sapiens" ;  
#---------------------------------------------------------------------------------------------------------------------------
#Step 1.Remove the first line of fasta files,and remove "enter".
#---------------------------------------------------------------------------------------------------------------------------
open (hand2, "./GRCh38_latest_genomic.fna") or die $!;
while (<hand2>)   {
	$_=~ s/\s+$//;
...
while (<REF>)   {
   ...
   my $cds_count = 0;
   my $utr_count = 0;
   print "processing $a[0]\n";                               #View progress,$a[11] is geneName.
...
#----------------------------------------------------------------------------------------------------------------------------------------------
	...
	if ($protein !~ /X|^.+\*.+$/ and $protein =~ /^.+\*$/)  {               # When three codons do not contain X , but end with stop codon.
	
		print Seq ">$a[0]\n$seq\n";                                      # Output Seq form: geneName\sequence
		print P ">$a[0]\n$protein\n";                                    # Output P form:geneName\corresponding protein
		# count codons in proteins that are 
		# 1, stop with stop codons, start codon is not limited to ATG.

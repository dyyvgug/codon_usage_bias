...
parser = argparse.ArgumentParser(description='calculates codon frequency,RSCU value,weight',prog = 'RSCU_weight',usage = '%(prog)s [options]') 
parser.add_argument('--inp', nargs='?',type=str, help='expected FASTA file of calculations RSCU and weight') 
args = parser.parse_args()
 
os.chdir('/media/hp/disk2/DYY2/cell_line/HEK293/experiment1/')
aa_codon = {
...
out_file = open('hE_codon_fre_RSCU.txt','w')
...
r_script = '''
hE_hT_gene_fre = read.table("hE_codon_fre_RSCU.txt",header = T,sep = '\t',quote = "")
df <- hE_hT_gene_fre
df$Weights <- ave(df$RSCU,df$AA,FUN=function(x) x/max(x)) # calculate weight
df = df[,-c(1,3,4,5)]
df = df[-c(30,61,62,63,64),]
write.table(df,file = "hE_RSCU_weight.txt",sep = '\t',quote = F,row.names = F,col.names = F) # for calculate CAI
'''
robjects.r(r_script)

...
spA = "Hs"
spe = "hg38"
setwd(paste0("/media/hp/disk2/DYY2/tRNA/",spA,"/seq(HEK293)"))
pathw = paste0("/media/hp/disk2/DYY2/tRNA/",spA,"/seq(HEK293)/ribo/")
tRNA_array = list.files(getwd(),pattern = "txt$") 
tRNA_array
for(i in tRNA_array){
  if(FALSE){
    tRNA = read.table('GSM1624818.txt',sep = '\t',header = T)
    name = 'GSM1624818'
  }
  ...
  rscu = read.table(paste0(pathw,"ribo_codon_fre_RSCU.txt"),
  ...
  anti_rscu_p = cor.test(log2(tRNA_rscu$Frq),log2(tRNA_rscu$hits))
  anti_rscu_p
  
  p3 <- ggplot(tRNA_rscu,aes(x = log2(tRNA_rscu$Frq),y = log2(tRNA_rscu$hits),color = tRNA_rscu$AA))+
    geom_point(shape = 16,size = 2.5)+
    labs(title = paste0(spA," cor_anticodon_codon    ","r=",round(anti_rscu_p$estimate,5),"  p=",round(anti_rscu_p$p.value,5)))+
    annotation_logticks(sides="bl")+
    stat_smooth(method="lm", se=FALSE,linetype="dashed", color = "black",size = 0.75)+
    ...
    ylab("log2 (high expression level genes codon count)")+
    theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
          axis.title.x =element_text(size=15), axis.title.y=element_text(size=15))
  p3
  ggsave(p3,filename = paste0(pathw,'hE_codon_re_dis_anticodon.pdf'))
  ...
  anti_rscu_p = cor.test(log2(tRNA_whole$Frq),log2(tRNA_whole$whole_fre))
  anti_rscu_p
  
  p4 <- ggplot(tRNA_whole,aes(x = log2(tRNA_whole$Frq),y = log2(tRNA_whole$whole_fre),color = tRNA_whole$aa))+
    geom_point(shape = 16,size = 2.5)+
    labs(title = paste0(spA," cor_anticodon_codon    ","r=",round(anti_rscu_p$estimate,5),"  p=",round(anti_rscu_p$p.value,5)))+
    annotation_logticks(sides="bl")+
    stat_smooth(method="lm", se=FALSE,linetype="dashed", color = "black",size = 0.75)+
    theme_bw()+
    xlab("log2 (anticodon count)")+
    ylab("log2 (whole genome codon count)")+
    theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
          axis.title.x =element_text(size=15), axis.title.y=element_text(size=15))
  p4

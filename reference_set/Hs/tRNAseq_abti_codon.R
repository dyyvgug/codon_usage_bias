...
pathw = paste0("/media/hp/disk2/DYY2/tRNA/",spA,"/seq(HEK293)/hEhT/")
...
for(i in tRNA_array){
  ...
  rscu = read.table(paste0(pathw,"hE_hT_codon_fre_RSCU.txt"),
  ...
  if(FALSE){
    ...
    anti_rscu_p = cor.test(tRNA_rscu$Frq,tRNA_rscu$hits)
    anti_rscu_p
    p3 <- ggplot(tRNA_rscu,aes(x = tRNA_rscu$Frq,y = tRNA_rscu$hits,color = tRNA_rscu$AA))+
      geom_point(shape = 16,size = 2.5)+
      labs(title = paste0(spA," cor_anticodon_codon    ","r=",round(anti_rscu_p$estimate,5),"  p=",round(anti_rscu_p$p.value,5)))+
      scale_x_log10(breaks = trans_breaks("log10", function(x) 10^x),
                    labels = trans_format("log10", math_format(10^.x))) +

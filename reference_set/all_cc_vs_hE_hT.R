...
png('all_cc_vs_hE_hT_TPMbox.png')
p <- boxplot(log(s_TPM$TPM),log(hE_hT$TPM),
             names=c('all_cc','hE_hT'),col=c("green","red"),ylab = 'log(TPM)')
dev.off()
png('all_cc_vs_hE_hT_riboTPMbox.png')
p <- boxplot(log(s_TPM$ribo_TPM),log(hE_hT$ribo_TPM),
             names=c('all_cc','hE_hT'),col=c("green","red"),ylab = 'log(ribo_TPM)')
dev.off()

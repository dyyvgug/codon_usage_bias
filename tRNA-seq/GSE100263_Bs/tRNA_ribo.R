...
count_array = list.files(getwd(),pattern = ".csv$") 
count_array
for (i in count_array) {
  if(FALSE){
    tRNA_count = read.csv('Bs_noDM.csv',header = F)
    name = 'Bs_noDM'
  }
  tRNA_count = read.csv(i,header = F)
  name = sub("^([^.]*).*", "\\1",i) 
  ...
  tRNA_ribo = tRNA_ribo[-11,]
  tRNA_ribo = tRNA_ribo[complete.cases(tRNA_ribo),]

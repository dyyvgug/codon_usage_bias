#==========================================================================================
# Yingying Dong.2019-12-18.modify:2022.tRNA seq anticodon & the reference set codon
#==========================================================================================
library(dplyr)
...
for (i in rscu_array) {
  ...
  if(i == "Mitochondria RP"){
    j = rgb(200,175,212,maxColorValue = 255)
  }
  if(i == "HTT"){
    j = rgb(252,215,24,maxColorValue = 255)
  }
  ...
  # correlation between anti-codon frequence and codon frequence
  anti_rscu_p = cor.test(log2(bed_rscu$anti_fre),bed_rscu$hits)
  anti_rscu_p
  
  # correlation between anti-codon frequence and RSCU
  anti_rscu_p_rs = cor.test(log2(bed_rscu$anti_fre),bed_rscu$RSCU)
  anti_rscu_p_rs
  
  p1 <- ggplot(bed_rscu,aes(x = log2(bed_rscu$anti_fre),y = bed_rscu$RSCU))+
    geom_point(shape = 16,size = 2.5,color = j)+
    labs(title = paste0(spA," cor_anticodon_codon    ","r=",round(anti_rscu_p_rs$estimate,5),"  p=",round(anti_rscu_p_rs$p.value,5)))+
    ...
    ylab(paste0("RSCU of ",i))+
    theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
          axis.title.x = element_text(size=16), axis.title.y = element_text(size=16),
          axis.text = element_text(size=14))
  p1
  ggsave(paste0(i,"_anticodon_RSCU.pdf"),width = 15, height = 10, units = "cm")
  write.csv(bed_rscu,paste0(i,"_anti_rscu.csv"),quote = F,row.names = F)
}


# whole genome genes
whole = read.table("CDS_all_codon_fre_RSCU.txt",header = T)
...
anti_whole_rs = cor.test(log2(bed_whole$anti_fre),bed_whole$RSCU)
anti_whole_rs

p2 <- ggplot(bed_whole,aes(x = log2(bed_whole$anti_fre),y = bed_whole$RSCU))+
  geom_point(shape = 16,size = 2.5,color = rgb(101,200,208,maxColorValue = 255))+
  labs(title = paste0(spA," cor_anticodon_genome_codon    ","r=",round(anti_whole_rs$estimate,5),"  p=",round(anti_whole_p$p.value,5)))+
  scale_y_log10(breaks = trans_breaks("log10", function(x) 10^x),
                labels = trans_format("log10", math_format(10^.x))) +
  annotation_logticks(sides="bl")+
  stat_smooth(method="lm", se=FALSE,linetype="dashed", color = "black",size = 0.75)+
  theme_bw()+
  xlab("Log2 (anticodon counts)")+
  ylab("RSCU of Whole genome")+
  theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(),
        axis.title.x =element_text(size=16), axis.title.y=element_text(size=16),
        axis.text = element_text(size = 14))
p2
ggsave("Sc_genome_codon_RSCU.pdf",width = 15,height = 10,units = "cm")
write.csv(bed_rscu,"tRNAseq_anti_genome_codon_rscu.txt",quote = F,row.names = F)
